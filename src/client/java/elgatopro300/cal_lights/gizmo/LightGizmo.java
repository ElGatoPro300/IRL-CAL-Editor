...
public class LightGizmo {
    ...
    public void init() {
        ...
        WorldRenderEvents.AFTER_TRANSLUCENT.register(this::renderFlares);
    }
    ...
    private void renderFlares(WorldRenderContext context) {
        MinecraftClient client = MinecraftClient.getInstance();
        if (client.world == null) return;
        if (CalLightsIcons.FLARE_TEXTURE == null) return;
        ...
        RenderSystem.setShader(GameRenderer::getPositionTexColorProgram);
        CalLightsIcons.FLARE_TEXTURE.bind();
        ...
        drawFlareBillboards(stack, camPos, camera, points);
        drawFlareBillboards(stack, camPos, camera, spots);
    ...
    private void drawFlareBillboards(MatrixStack stack, Vec3d camPos, Camera camera, Collection<LightInstance> lights) {
        Vec3d camDir = Vec3d.fromPolar(camera.getPitch(), camera.getYaw());
        Quaternionf camRot = camera.getRotation();

        for (LightInstance light : lights) {
            if (!light.visible || !light.flareEnabled) {
                continue;
            }

            Vec3d toLight = new Vec3d(light.x - camPos.x, light.y - camPos.y, light.z - camPos.z);
            double dist = toLight.length();
            if (dist < 0.01) {
                continue;
            }

            Vec3d toLightNorm = toLight.normalize();
            double dot = camDir.dotProduct(toLightNorm);
            double angle = Math.toDegrees(Math.acos(Math.max(-1.0, Math.min(1.0, dot))));

            if (angle > light.flareEndAngle) {
                continue;
            }

            float angleFade = 1.0f;
            if (angle > light.flareStartAngle) {
                double range = light.flareEndAngle - light.flareStartAngle;
                if (range > 0.01) {
                    angleFade = (float) (1.0 - (angle - light.flareStartAngle) / range);
            ...
            int rInt = Math.max(0, Math.min(255, (int) (light.r * 255)));
            int gInt = Math.max(0, Math.min(255, (int) (light.g * 255)));
            int bInt = Math.max(0, Math.min(255, (int) (light.b * 255)));

            float intensityFactor = Math.max(0.0f, Math.min(1.0f, light.intensity));
            float finalAlpha = intensityFactor * angleFade;
            ...
            // 1. Draw Glow
            if (light.flareGlowSize > 0.01f && light.flareGlowIntensity > 0.01f && CalLightsIcons.FLARE_GLOW_TEXTURE != null) {
                CalLightsIcons.FLARE_GLOW_TEXTURE.bind();
                float glowSize = 0.5f * light.flareGlowSize;
                int glowAlphaInt = Math.max(0, Math.min(255, (int) (finalAlpha * light.flareGlowIntensity * 255)));

                Matrix4f matrix = stack.peek().getPositionMatrix();
                BufferBuilder builder = Tessellator.getInstance().begin(VertexFormat.DrawMode.QUADS,
                        VertexFormats.POSITION_TEXTURE_COLOR);

                builder.vertex(matrix, -glowSize, -glowSize, 0).texture(0.0f, 1.0f).color(rInt, gInt, bInt, glowAlphaInt);
                builder.vertex(matrix, glowSize, -glowSize, 0).texture(1.0f, 1.0f).color(rInt, gInt, bInt, glowAlphaInt);
                builder.vertex(matrix, glowSize, glowSize, 0).texture(1.0f, 0.0f).color(rInt, gInt, bInt, glowAlphaInt);
                builder.vertex(matrix, -glowSize, glowSize, 0).texture(0.0f, 0.0f).color(rInt, gInt, bInt, glowAlphaInt);

                BufferRenderer.drawWithGlobalProgram(builder.end());
            }

            // 2. Draw Primary Ray
            if (light.flareRayLength > 0.01f && light.flareRayThickness > 0.01f && CalLightsIcons.FLARE_RAY_TEXTURE != null) {
                CalLightsIcons.FLARE_RAY_TEXTURE.bind();
                stack.push();
                stack.multiply(new Quaternionf().rotateZ((float) Math.toRadians(light.flareRotation)));

                float halfLen = 0.5f * light.flareRayLength;
                float halfThick = 0.5f * light.flareRayThickness;
                int rayAlphaInt = Math.max(0, Math.min(255, (int) (finalAlpha * 255)));

                Matrix4f matrix = stack.peek().getPositionMatrix();
                BufferBuilder builder = Tessellator.getInstance().begin(VertexFormat.DrawMode.QUADS,
                        VertexFormats.POSITION_TEXTURE_COLOR);

                builder.vertex(matrix, -halfLen, -halfThick, 0).texture(0.0f, 1.0f).color(rInt, gInt, bInt, rayAlphaInt);
                builder.vertex(matrix, halfLen, -halfThick, 0).texture(1.0f, 1.0f).color(rInt, gInt, bInt, rayAlphaInt);
                builder.vertex(matrix, halfLen, halfThick, 0).texture(1.0f, 0.0f).color(rInt, gInt, bInt, rayAlphaInt);
                builder.vertex(matrix, -halfLen, halfThick, 0).texture(0.0f, 0.0f).color(rInt, gInt, bInt, rayAlphaInt);

                BufferRenderer.drawWithGlobalProgram(builder.end());
                stack.pop();
            }

            // 3. Draw Secondary Ray
            if (light.flareRayLength2 > 0.01f && light.flareRayThickness2 > 0.01f && CalLightsIcons.FLARE_RAY_TEXTURE != null) {
                CalLightsIcons.FLARE_RAY_TEXTURE.bind();
                stack.push();
                stack.multiply(new Quaternionf().rotateZ((float) Math.toRadians(light.flareRotation + 90.0f)));

                float halfLen = 0.5f * light.flareRayLength2;
                float halfThick = 0.5f * light.flareRayThickness2;
                int rayAlphaInt = Math.max(0, Math.min(255, (int) (finalAlpha * 255)));
            ...
            // 4. Draw Diagonal Ray 1 (45 degrees)
            if (light.flareRayLength3 > 0.01f && light.flareRayThickness3 > 0.01f && CalLightsIcons.FLARE_RAY_TEXTURE != null) {
                CalLightsIcons.FLARE_RAY_TEXTURE.bind();
                stack.push();
                stack.multiply(new Quaternionf().rotateZ((float) Math.toRadians(light.flareRotation + 45.0f)));

                float halfLen = 0.5f * light.flareRayLength3;
                float halfThick = 0.5f * light.flareRayThickness3;
                int rayAlphaInt = Math.max(0, Math.min(255, (int) (finalAlpha * 255)));
            ...
            // 5. Draw Diagonal Ray 2 (135 degrees)
            if (light.flareRayLength3 > 0.01f && light.flareRayThickness3 > 0.01f && CalLightsIcons.FLARE_RAY_TEXTURE != null) {
                CalLightsIcons.FLARE_RAY_TEXTURE.bind();
                stack.push();
                stack.multiply(new Quaternionf().rotateZ((float) Math.toRadians(light.flareRotation + 135.0f)));
